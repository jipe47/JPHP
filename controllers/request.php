	...
	public function handler()
	{
		switch($this->arg->string(0))
		{
			...
			case "login":
				...
				$login = Post::string("login");
				$password = Post::string("password");
				$remember = Post::boolValue("remember", "on", false);
				
				$info_login = $this->request->firstQuery("SELECT * FROM admin WHERE login='" . $login . "'");
			...
			case "login_cookie":
				...
				$login = Cookie::string(STRUCTURE_NAME."_login");
				$password = Cookie::string(STRUCTURE_NAME."_password");
				$info_login = $this->request->firstQuery("SELECT * FROM admin WHERE login='" . $login . "'");
