...
class TreeNode
{
	public $id, $value, $children, $level, $id_parent;
	
	public function __construct($id = 0, $value = "", $level = 0, $id_parent = -1)
	{
		$this->id = $id;
		$this->value = $value;
		$this->children = array();
	...
	{
		$a = array('id' => $this->id, 'value' => $this->value, 'children' => array(), 'level' =>  $this->level, 'id_parent' => $this->id_parent);
	...
	public function addNode($id, $value, $id_parent, $level = 0)
	{
		...
		{
			$this->children[] = new TreeNode($id, $value, $this->level + 1, $id_parent);
			return true;
		}
		$b = false;
		foreach($this->children as $c)
