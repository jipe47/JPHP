...
class HtmlHeaders
{
	...
	private static $sessionVar = "jphp_htmlheaders_cache";
	...
	{
		//$_SESSION[self::$sessionVar] = serialize(array("paths" => self::$paths));
		Cache::write("htmlheaders", array("paths" => self::$paths));
	}
	...
	*/
	...
	private static $cache_reset = false;
	/**
	...
	*/
	public static function includeDir($type, $path)
	{
		...
		{
			$method = $type == "css" ? "addCssFile" : "addJsSource";
			foreach(self::$paths[$path] as $f) 
				self::$method($path."/".$f);
		}
		else
		{
			...
			//echo 'Including dir ' . $path . ' of type ' . $type . '<br />';
			$deque = new Deque();
			$deque->enqueue($path);
