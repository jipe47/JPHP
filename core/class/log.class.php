...
class Log
{
	...
	{
		self::$pageId = jphp_generateId(10);
	 ...
	 */
	public static function add($message, $type = Message::SUCCESS)
	{
		self::getLogObject();
		
		$e = array(	/*"sessionid" => jphp_getSessionId(),*/
									"timestamp"	=> time(),
									"type" => $type,
									"message" => $message,
	...
	{
		$_SESSION['jphp_log'] = serialize(self::$entries);
	}
	...
	{
		if(!empty($_SESSION['jphp_log']) && !self::$getFromSession)
			self::$entries = unserialize($_SESSION['jphp_log']);
		else
			self::$entries = array();
