			...
			{	
				...
				if($file['size'] <= $maxsize)
				{
					...
					if(self::ALLOW_ALL || array_key_exists($extension, self::$allowed_extension) && self::$allowed_extension[$extension] == $file['type'])
					{
						...
						{
							Log::add("The file has not been moved.", Message::ERROR);
							return self::ERROR_MOVE;
						}
					}
					else
					{
						Log::add("Extension not allowed or wrong MIME type.", Message::ERROR);
						return self::ERROR_EXTENSION;
					}
				}
				else
				{
					Log::add("File too big (max " . $maxsize . " bits.", Message::ERROR);
					return self::ERROR_TOOBIG;
				}
			}
			else
			{
				Log::add("Upload error: ".self::$errors[$_FILES[$field]['error']], Message::ERROR);
				return self::ERROR_UPLOAD;
			}
		}
		else
		{
			Log::add("The field '".$field . "' is not set.", Message::ERROR);
