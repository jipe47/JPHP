...
class Messages
{
	 ...
	 * @see Message
	 */
	public static function add($text, $type = Message::SUCCESS)
	{
	...
	public static function render()
	{
		...
		foreach($a as $m)
		{
			//echo 'Rendering "'.$m['text'].'"<br/>';
			$c = new Message($m['text'], $m['type']);
			$r .= $c->render();
		}
		return $r;
	}
	...
	*/
	...
	{
		$_SESSION['jphp_messages'] = serialize(self::$messages);
	}
	...
	*/
	private static function loadMessages()
	{
		...
		if(!empty($_SESSION['jphp_messages']))
			self::$messages = unserialize($_SESSION['jphp_messages']);
	}
