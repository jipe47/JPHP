	...
	public static function removeIntKeys($array)
	{
		foreach($array as $k => $v)
		{
			if(is_numeric($k))
				...
				$array[$k] = self::removeIntKeys($v);
		}
		return $array;
	}
	
	public static function makeSqlTimestamp($time)
	{
		if(is_array($time)) // TODO Handle hours, minutes and seconds
			return $time['year']."-".str_pad($time['month'], 2, "0", STR_PAD_LEFT)."-".str_pad($time['day'], 2, "0", STR_PAD_LEFT) . " 00:00:00";
		else
