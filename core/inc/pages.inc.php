...
function displayPage($i, $page, $back)
{
	if($i == $page)
		...
		return '<a href="?'.$back.JPHP::ARG_SEPARATOR.$i. '">' . $i . '</a> ';
}
function generatePages($nbr_page, $page, $back)
{
	$url = $back.JPHP::ARG_SEPARATOR;
	...
	echo '<a href="?'. $url . $next_page . '">></a> ';
	echo '<a href="?'. $url . $nbr_page . '">|></a> ';
}